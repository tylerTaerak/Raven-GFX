...
create_descriptor_layout :: proc(device : Device, desc_configs : []core.Descriptor_Type) -> vk.DescriptorSetLayout {
    ...
    layout : vk.DescriptorSetLayout
    vk.CreateDescriptorSetLayout(device.core, &layout_info, {}, &layout)

    return layout
}

create_descriptor_sets :: proc(device : Device,
    set_count : int,
    ...
    for s_idx in 0..<set_count {
        ...
        set.layout = create_descriptor_layout(device, cfg)
        ...
        vk.GetDescriptorSetLayoutSizeEXT(device.core, set.layout, &required_size)
        ...
        for binding_idx in 0..<len(cfg) {
            ...
            vk.GetDescriptorSetLayoutBindingOffsetEXT(device.core, set.layout, u32(binding_idx), &required_offset)
...
// write_descriptor_buffer :: proc(ctx : ^Context, descriptors : []Descriptor_Set, set_index : int, binding_index : int, write_data : gmem.Gpu_Slice, arena : ^gmem.Gpu_Arena) {
//     desc_buf_props : vk.PhysicalDeviceDescriptorBufferPropertiesEXT
//     desc_buf_props.sType = .PHYSICAL_DEVICE_DESCRIPTOR_BUFFER_PROPERTIES_EXT
// 
//     dev_props : vk.PhysicalDeviceProperties2KHR
...
// bind_descriptor_sets :: proc(device : Device, cmd_buf : vk.CommandBuffer, desc_sets : []Descriptor_Set, layout : vk.PipelineLayout, ) {
...
//         binding_info.address = gmem.device_pointer({device.core, device.physical, {}}, desc_sets[i].buffer.block^)
...
destroy_descriptor_sets :: proc(device : Device, sets : []Descriptor_Set) {
    for i in 0..<len(sets) {
        delete(sets[i].bindings)
        vk.DestroyDescriptorSetLayout(device.core, sets[i].layout, {})
    }
